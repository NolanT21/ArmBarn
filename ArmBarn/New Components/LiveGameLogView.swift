...
struct LiveGameLogData: Hashable {
    var pitch_num: Int
    var batter_num: Int
...
struct InningSummaryData: Hashable {
    var pitches: Int
...
//                                                    : -gr.frame(in: .global).origin.y) // Push it up!
                ...
                if events.count <= 0 {
                    Text("No Events have been recorded")
    ...
    func generate_game_log_data() {
        ...
        for event in events {
            ...
            let game_log_datarow = LiveGameLogData(pitch_num: pitch_num, batter_num: event.atbats, batter_stance: event.batter_stance, inning: event.inning, result: result, result_detail: event.result_detail, pitch_type: cur_pitch_type, velo: event.velocity, balls: event.balls, strikes: event.strikes, outs: outs, x_location: x_location, y_location: y_location, pitch_color: result_color, pitcher_name: pitcher_name)
